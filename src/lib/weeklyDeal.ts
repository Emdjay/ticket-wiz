...
export type WeeklyDealResult = {
  context: WeeklyDealContext;
  top: { offer: FlightOffer; destination: string; score: number };
  totalDurationMinutes: number;
  maxStops: number;
};

export type WeeklyDealOptions = {
  token?: string;
  baseUrl?: string;
};

export async function getWeeklyDeal(
  options: WeeklyDealOptions = {}
): Promise<WeeklyDealResult | null> {
  ...
  const token = options.token ?? (await getAmadeusAccessToken());
  const baseUrl = options.baseUrl ?? getAmadeusBaseUrl();
  ...
  const totalDurationMinutes = top.offer.itineraries.reduce((sum, it) => {
